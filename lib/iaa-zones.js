  ...
  for (const [k, newF] of after) {
    ...
    for (const w of WATCHED) {
      ...
      if (JSON.stringify(a) !== JSON.stringify(b)) {
        fields.push(w);
        details.push({ field: w, from: a == null ? '—' : String(a), to: b == null ? '—' : String(b) });
      }
    }
    if (geomHash(oldF) !== geomHash(newF)) {
      fields.push('geometry');
      details.push({ field: 'geometry', from: 'boundary moved', to: 'see the dataset' });
    }

    // Time windows decide whether a zone is live for a given flight, so a change
    // here matters even though nothing else moved.
    const win = (f) => ((f.properties || {}).limitedApplicability || []);
    const oldWin = JSON.stringify(win(oldF));
    const newWin = JSON.stringify(win(newF));
    if (oldWin !== newWin) {
      fields.push('limitedApplicability');
      const fmt = (arr) => arr.length
        ? arr.map(w => `${(w.startDateTime || '?').slice(0, 16).replace('T', ' ')} → ${(w.endDateTime || '?').slice(0, 16).replace('T', ' ')}`).join('; ')
        : 'always active';
      details.push({ field: 'active windows', from: fmt(win(oldF)), to: fmt(win(newF)) });
    }

    if (fields.length) {
      changed.push({ id: k, name: newF.properties.name, fields, details });
    }
  }
    ...
    try {
      ...
      if (prev) {
        ...
        if (republished) {
          ...
          // Keep the full diff, not just the counts — the alert lists the zones,
          // and passing counts alone here broke that path.
          republishedDiff = diffVersions(JSON.parse(prevText), geo);
          entry.diff = republishedDiff.counts;
          if (!dryRun) {
            await supabase.storage.from(BUCKET).upload(
